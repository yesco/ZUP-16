// mini8.v: A mini8 stack cpu
//
// Editing: Only change lines if really needed, any other changes ask

// --- Master Global Architecture Definitions ---
`define BITSY   4'b0000
`define BITSY2  4'b0000
`define LOOP    4'b0010
`define LOOP2   4'b0011
`define JSRRET  4'b0100
`define JSRRET2 4'b0101
`define BRANCH  4'b0110
`define BRANCH2 4'b0111
`define READ    4'b1000
`define READ2   4'b1000
`define WRITE   4'b1010
`define WRITE2  4'b1011
`define REG     4'b1100
`define STACK   4'b1101
`define ALU     4'b1110
`define RSTACK  4'b1111

// BITSY Sub-instruction Opcodes
`define SIGN   3'b000
`define bit14  3'b001
`define bit13  3'b010
`define bit12  3'b011
`define bit11  3'b100
`define bit10  3'b101
`define bit09  3'b110
`define bit08  3'b111
`define bit07  3'b000
`define bit06  3'b001
`define bit05  3'b010
`define bit04  3'b011
`define bit03  3'b100
`define bit02  3'b101
`define bit01  3'b110
`define bit00  3'b111

// ALU Sub-instruction Opcodes
`define ADD  3'b000
`define ADC  3'b001
`define SUB  3'b010
`define SBC  3'b011
`define AND  3'b100
`define OR   3'b101
`define XOR  3'b110
`define DROP 3'b111

// Register Ops (not moving stack)
`define INC  3'b000
`define DEC  3'b001
`define ROR  3'b010
`define ASR  3'b011
`define SHR  3'b100
`define SHR4 3'b101
`define SHL  3'b110
`define SHL4 3'b111

// STK Sub-instruction Opcodes
`define st0  3'b000
`define MUL  3'b001
`define NOP  3'b010
`define ROT  3'b011
`define SWAP 3'b100
`define OVER 3'b101
`define TUCK 3'b110
`define DUP  3'b111

// --- Fused Mnemonic String + 8-Bit Opcode Macro Constants ---

// All these take 2 leaves 1 value
`define iADD  {"ADD ", {1'b1, `ALU,   `ADD}}
`define iADC  {"ADC ", {1'b1, `ALU,   `ADC}}
`define iSUB  {"SUB ", {1'b1, `ALU,   `SUB}}
`define iSBC  {"SBC ", {1'b1, `ALU,   `SBC}}
`define iAND  {"AND ", {1'b1, `ALU,   `AND}}
`define iOR   {"OR  ", {1'b1, `ALU,   `OR}}
`define iXOR  {"XOR ", {1'b1, `ALU,   `XOR}}
`define iDROP {"DROP", {1'b1, `ALU,   `DROP}}

// There are mixed when come to stack
`define ist0  {"st0 ", {1'b1, `STACK, `st0}}
`define iMUL  {"MUL ", {1'b1, `STACK, `MUL}}
`define iNOP  {"NOP ", {1'b1, `STACK, `NOP}}
`define iROT  {"ROT ", {1'b1, `STACK, `ROT}}
`define iSWAP {"SWAP", {1'b1, `STACK, `SWAP}}
`define iOVER {"OVER", {1'b1, `STACK, `OVER}}
`define iTUCK {"TUCK", {1'b1, `STACK, `TUCK}}
`define iDUP  {"DUP ", {1'b1, `STACK, `DUP}}

// Register style modification, stack still
`define iINC  {"INC ", {1'b1, `REG,   `INC}}
`define iDEC  {"DEC ", {1'b1, `REG,   `DEC}}
`define iROR  {"ROR ", {1'b1, `REG,   `ROR}}
`define iASR  {"ASR ", {1'b1, `REG,   `ASR}}
`define iSHR  {"SHR ", {1'b1, `REG,   `SHR}}
`define iSHR4 {"SHR4", {1'b1, `REG,   `SHR4}}
`define iSHL  {"SHL ", {1'b1, `REG,   `SHL}}
`define iSHL4 {"SHL4", {1'b1, `REG,   `SHL4}}

`define iSIGN   {"SIGN", {1'b1, `BITSY, `SIGN}}
`define ibit14  {"bt14", {1'b1, `BITSY, `bit14}}
`define ibit13  {"bt13", {1'b1, `BITSY, `bit13}}
`define ibit12  {"bt12", {1'b1, `BITSY, `bit12}}
`define ibit11  {"bt11", {1'b1, `BITSY, `bit11}}
`define ibit10  {"bt10", {1'b1, `BITSY, `bit10}}
`define ibit09  {"bt09", {1'b1, `BITSY, `bit09}}
`define ibit08  {"bt08", {1'b1, `BITSY, `bit08}}

`define ibit07  {"bt07", {1'b1, `BITSY2, `bit07}}
`define ibit06  {"bt06", {1'b1, `BITSY2, `bit06}}
`define ibit05  {"bt05", {1'b1, `BITSY2, `bit05}}
`define ibit04  {"bt04", {1'b1, `BITSY2, `bit04}}
`define ibit03  {"bt03", {1'b1, `BITSY2, `bit03}}
`define ibit02  {"bt02", {1'b1, `BITSY2, `bit02}}
`define ibit01  {"bt01", {1'b1, `BITSY2, `bit01}}
`define ibit00  {"bt00", {1'b1, `BITSY2, `bit00}}

// TODO: stack?
`define iJZ   {"JZ  ", {1'b1, `BRANCH,  3'b000}}
`define iJNZ  {"JNZ ", {1'b1, `BRANCH2, 3'b000}}

module mini8 (
              input  wire       clk, 
              input  wire       rst_n,
              output reg  [7:0] pc,
              input  wire [7:0] op,

              // External flag monitors
              output wire       c, 
              output wire       z, 
              output wire       n, 
              output wire       v  
              );

   // ==========================================================
   // 1. Storage & Wire Aliases (The Background Map)
   // ==========================================================
   reg        c_reg;
   reg [7:0]  tos;        
   reg [7:0]  nos;
   reg [7:0]  n2;

   assign acc  = tos;
   assign c_in = c_reg;

   assign c = c_reg;
   assign z = (tos == 8'd0); 
   assign n = tos;
   assign v = 0;

   // Instruction Decoder Extraction
   wire       is_lit   = (op[7] == 0); // DON'T CHANGE: you keep messing it up!
   wire [6:0] lit_data = op[6:0];      
   
   wire [3:0] grp    = op[6:3];          
   wire [2:0] sub_op = op[2:0];        

   // Interconnect Nets
   reg [7:0]  nxt_tos;
   reg        nxt_c;    
   reg [7:0]  nxt_pc;      

   // Factored Next-State Stack Nets
   reg [7:0]  nxt_nos;
   reg [7:0]  nxt_n2;

   // Structured stack pointer control assertions
   reg do_push = 0;
   reg do_pop  = 0;

   // --- Single-Adder Control Mux Nets ---
   reg [7:0]  b_mux;
   reg [7:0]  a_mux;
   reg        cin;

   // ==========================================================
   // 3. CONCERN A: Pure ALU Math & Stack Engine
   // ==========================================================
   always @(*) begin

      // Default values
      nxt_tos   = tos;
      nxt_nos   = nos;
      nxt_n2    = n2;
      nxt_c     = c_reg; 
      nxt_pc    = pc + 1;

      do_push = is_lit;
   

      // Shared Operand Route Baseline Defaults
      b_mux = tos;
      a_mux = nos;
      
      cin   = 0;

      if (is_lit) begin

         // PUSH logic embedded directly
         nxt_c   = 0;
         nxt_tos = {1'b0, lit_data};

      end else if (grp == `ALU || grp == `REG) begin

	 do_drop = 1;
	 
         // PASS 1: Set up the routing parameters for arithmetic operations
         case (sub_op)
           `ADD: begin b_mux = tos;  cin = 0;     end
           `ADC: begin b_mux = tos;  cin = c_reg; end
           `SUB: begin b_mux = ~tos; cin = 1;     end
           `SBC: begin b_mux = ~tos; cin = ~c_reg;end
         endcase 

         if (grp == `REG) begin 

	    do_drop = 0;

            case (sub_op)
              `INC: begin b_mux = tos; a_mux =  0;  cin = 1; end
              `DEC: begin b_mux = tos; a_mux = ~0;  cin = 0; end
            endcase

         end

         // THE ONLY ARITHMETIC LINE
         {nxt_c, nxt_tos} = a_mux + b_mux + cin;

         // PASS 2: Logical operations cleanly overwrite nxt_tos if active
         if (grp[1]) begin

            case (sub_op)
              `AND:  nxt_tos = tos & nos;
              `OR :  nxt_tos = tos | nos;
              `XOR:  nxt_tos = tos ^ nos;
              `DROP: nxt_tos = nos;
            endcase

         end else if (sub_op[2]) begin

	    do_drop = 0;
	    
            case (sub_op)
              `SHR:  begin nxt_tos = tos /  2; end 
              `SHR4: begin nxt_tos = tos / 16; end
              `SHL:  begin nxt_tos = tos *  2; end 
              `SHL4: begin nxt_tos = tos * 16; end 
            endcase
            
            if (grp == `REG && sub_op[2])
               nxt_c = sub_op[1] ? (sub_op[0] ? tos[3] : tos[7]) : (sub_op[0] ? tos[4] : tos[0]);

         end

      end else if (grp == `STACK) begin
         
         case (sub_op)
//         `OVER: begin do_push = 1; nxt_tos = nos;                end
//         `DUP:  begin do_push = 1;                               end
//	   `TUCK: begin do_push = 1; n2 = tos;                     end
//         `SWAP: begin do_pop = 0;     nxt_tos = nos; nxt_nos = tos;          end
//	   `ROT:  begin     nxt_tos = nos; nxt_nos = n2; n2 = tos; end
	       
	   `st0:  begin end
	   `MUL:  begin end
	   `NOP:  begin end // TODO:
           `ROT:  begin nxt_tos = n2;  nxt_nos = tos; nxt_n2 = nos; end
	   
           `SWAP: begin nxt_tos = nos; nxt_nos = tos;               end
           `OVER: begin nxt_tos = nos; nxt_nos = tos; nxt_n2 = nos; end // push
           `TUCK: begin nxt_tos = tos; nxt_nos = nos; nxt_n2 = tos; end // push
           `DUP:  begin nxt_tos = tos; nxt_nos = tos; nxt_n2 = nos; end // push

//           `DROP: begin nxt_tos = nos; nxt_nos = n2;                end // pop

           default: begin                                           end
         endcase	       
	      
      end else if (grp == `BRANCH) begin

         if (z) begin
            nxt_pc = nos;
         end

      end


      // Central Stack Adjustment
      // TODO: add backing stack SSRAM
      if (do_push) begin
         nxt_nos = tos;
         nxt_n2  = nos;
      end else if (do_drop) begin
         nxt_nos = n2;
         nxt_n2  = 0;
      end
   end


   // ==========================================================
   // 7. CONCERN E: Flattened Sequential Storage
   // ==========================================================
   always @(posedge clk or negedge rst_n) begin
      if (!rst_n) begin
         pc    <= 0;
         c_reg <= 0;
         tos   <= 0;
         nos   <= 0;
         n2    <= 0;
      end else begin
         pc    <= nxt_pc;    
         c_reg <= nxt_c; 
         tos   <= nxt_tos;   
         nos   <= nxt_nos;
         n2    <= nxt_n2;
      end
   end
endmodule
